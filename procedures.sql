...
CREATE PROCEDURE cancelproc(in T_ID int, OUT RET INT)
    BEGIN
    ...
    SELECT T_TYPE, C_ID FROM CLIENTS_TRANS WHERE T_ID = T_ID INTO T_TYPE, C_ID;
    IF T_TYPE = 2 THEN
        SELECT (0.0 - AMOUNT) FROM PAYMENTS WHERE T_ID = T_ID INTO CASH;
        DELETE FROM PAYMENTS WHERE T_ID = T_ID;
    ELSE
        IF T_TYPE = 0 THEN
            SELECT AMOUNT * PRICE + COMM_CASH FROM OIL_TRANSACTIONS INTO CASH;
            SELECT COMM_OIL - AMOUNT FROM OIL_TRANSACTIONS INTO OIL;
        ELSE
            SELECT COMM_CASH - AMOUNT * PRICE FROM OIL_TRANSACTIONS INTO CASH;
            SELECT AMOUNT + COMM_OIL FROM OIL_TRANSACTIONS INTO OIL;
        END IF;
        DELETE FROM OIL_TRANSACTIONS WHERE T_ID = T_ID;
    END IF;
    UPDATE CLIENTS SET OIL_BALAN = OIL_BALAN + OIL, CASH_BALAN = CASH_BALAN + CASH WHERE UID = C_ID;
    COMMIT;
END//
