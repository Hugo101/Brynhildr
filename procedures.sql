...
CREATE PROCEDURE searchmanager(IN MID INT, OUT RET INT)
    BEGIN
    SET RET = 0;
    SELECT EXISTS(SELECT UID FROM USERS WHERE UID=MID AND TYPE=2) INTO RET;
END//
...
CREATE PROCEDURE cancelproc(IN TID INT, IN TRID INT, OUT RET INT)
    BEGIN
    DECLARE OIL DECIMAL(65,4);
    DECLARE CASH DECIMAL(65,4);
    DECLARE TTYPE INT;
    DECLARE CID INT;
    START TRANSACTION;
    SET CASH = 0.0;
    ...
    SELECT T_TYPE, C_ID FROM CLIENTS_TRANS WHERE T_ID = TID INTO TTYPE, CID;
    IF TTYPE IS NULL THEN
        SET RET = -2;
    END IF;
    IF TTYPE = 2 THEN
        SELECT (0.0 - AMOUNT) FROM PAYMENTS WHERE T_ID = TID INTO CASH;
        DELETE FROM PAYMENTS WHERE T_ID = TID;
    ELSE
        IF TTYPE = 0 THEN
            SELECT AMOUNT * PRICE + COMM_CASH FROM OIL_TRANSACTIONS WHERE T_ID = TID INTO CASH;
            SELECT COMM_OIL - AMOUNT FROM OIL_TRANSACTIONS WHERE T_ID = TID INTO OIL;
        ELSE
            SELECT COMM_CASH - AMOUNT * PRICE FROM OIL_TRANSACTIONS WHERE T_ID = TID INTO CASH;
            SELECT AMOUNT + COMM_OIL FROM OIL_TRANSACTIONS WHERE T_ID = TID INTO OIL;
        END IF;
        DELETE FROM OIL_TRANSACTIONS WHERE T_ID = TID;
    END IF;
    UPDATE CLIENTS SET OIL_BALAN = OIL_BALAN + OIL, CASH_BALAN = CASH_BALAN + CASH WHERE UID = CID;
    COMMIT;
END//

CREATE PROCEDURE wavepriceproc()
    BEGIN
    DECLARE CUR_PRICE DECIMAL(65,4);
    START TRANSACTION;
    SELECT PRICE FROM OILPRICES ORDER BY PRICE_DATE DESC LIMIT 1 INTO CUR_PRICE;
    INSERT INTO OIL_PRICES VALUES (CURDATE(), TRUNCATE(CUR_PRICE * (1 + SIN(RAND()-RAND())*0.1), 2));
    COMMIT;
END//
